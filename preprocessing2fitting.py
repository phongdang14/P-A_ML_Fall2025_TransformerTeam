...
def convert_mag2flux_columns(df, metadata_cols):
    ...
    Returns (df, mag_cols)
    """
    mag_cols = [c for c in df.columns if c not in metadata_cols and not c.endswith("_err") and not c.endswith("_flux")]
...
def fit_band(times, fluxes):
    ...
    try:
        popt, pcov = curve_fit(model1, t, f, p0=p0, bounds=(lower, upper), maxfev=20000)
...
def plot_flux_fit(times, flux, popt, band, sn_name, save_path=None, show=True):
    ...
    if popt is not None and np.all(np.isfinite(popt)):
        plt.plot(tfit, model1(tfit, *popt), 'r-', lw=2, label="Model fit")
    else:
...
def process_sn_file(input_path, group, sn_file, target_bands=TARGET_BANDS, output_dir=None, plot_each=False):
    """
    Process one file:
      - load data
      - convert mags -> flux
    ...
    if len(bands) == 0:
        ...
        return pd.DataFrame(columns=["sn_name", "sn_type", "band", "A", "phi", "sigma", "k"])

    # basic SN info
    sn_name = df["sn_name"].iloc[0] if "sn_name" in df.columns else os.path.splitext(sn_file)[0]
    ...
    for band in bands:
        ...
        times = df[time_col].values if time_col in df.columns else df.get("MJD", df.get("time")).values
        ...
        if n_finite < MIN_POINTS_TO_FIT:
            print(f"  Skipping {band}: only {n_finite} finite points (need {MIN_POINTS_TO_FIT})")
            continue
        ...
        if popt is None:
            row = {"sn_name": sn_name, "sn_type": sn_type, "band": band, "A": np.nan, "phi": np.nan, "sigma": np.nan, "k": np.nan}
        else:
            row = {"sn_name": sn_name, "sn_type": sn_type, "band": band, "A": popt[0], "phi": popt[1], "sigma": popt[2], "k": popt[3]}
        results.append(row)
        ...
        plot_flux_fit(times, flux, popt if ok else None, band, sn_name, save_path=save_png, show=plot_each)

    if results:
        return pd.DataFrame(results, columns=["sn_name", "sn_type", "band", "A", "phi", "sigma", "k"])
    else:
        return pd.DataFrame(columns=["sn_name", "sn_type", "band", "A", "phi", "sigma", "k"])

# ---------- 8) Build master table ----------
def build_master_ml_table(input_path, group, output_csv=None, target_bands=TARGET_BANDS, output_dir=None, plot_each=True):
    """
    ...
    files = [f for f in os.listdir(folder) if os.path.isfile(os.path.join(folder, f)) and f.endswith((".csv", ".dat"))]
    if not files:
    ...
    for fn in files:
        print("----")
        print("File:", fn)
        df_res = process_sn_file(input_path, group, fn, target_bands=target_bands, output_dir=output_dir, plot_each=plot_each)
        if not df_res.empty:
    ...
    if all_dfs:
        ...
        master = pd.DataFrame(columns=["sn_name", "sn_type", "band", "A", "phi", "sigma", "k"])
...
if __name__ == "__main__":
    ...
    output_files = ['master_ML_table_TypeIa.csv', 'master_ML_table_TypeIbc.csv', 'master_ML_table_TypeII.csv', 'master_ML_table_TypeIIn.csv']
    ...
    for group, output_csv in zip(groups, output_files):
        ...
        if group == 'IIn':
            ...
            master_df = build_master_ml_table(input_path, group, output_csv=output_filepath, target_bands=TARGET_BANDS, output_dir=output_png, plot_each=True)
